...
SECTIONS_PER_BODY_PAGE_DEFAULT = 4      # 빈 ◦/목차점선 정리(clean_lone_markers·clean_empty_toc)로
                                        # 절 단위가 짧아져 한 쪽에 ~4절. (정리 전 패딩 기준 3 → 4)
...
def make_values_for_toc_pages(skeleton_mapping_path: str,
    ...
    for s in toc_slots:
        ...
        if s["original_text"].strip().isdigit():
            ...
            # 제목 슬롯이 미사용(EMPTY_MARKER/빈)이면 이후 페이지번호 슬롯을 채우지 않아
            # 빈 목차 점선(…N)이 남지 않게 한다(current_chapter=0 → chapter_pages 미포함).
            if "EMPTY_PLACEHOLDER" in user_text or not user_text.strip():
                current_chapter = 0
    return values
