...
def fill_skeleton(skeleton_path: Path, values: dict, output_path: Path,
    ...
    is_full = "format_full" in str(skeleton_path)
    ...
    if expand_body:
        if is_one_pager:
            values, sec, body_summary = apply_1p_expansion(values, sec)
        elif is_full:
            values, sec, body_summary = apply_full_expansion(values, sec)
        else:
            values, sec, body_summary = apply_body_expansion(values, sec)
    # v3.7.0: 1p 는 배열 확장으로 ◦ 슬롯이 채워진 뒤 마커 정규화 (◦ 자동 보정)
