...
export default async function SafetyLibraryPage() {
  await connectDb();
  const items = await SafetyArticleModel.find({ type: "library" }).sort({ createdAt: -1 }).limit(50).lean();
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <Link href="/safety" className="text-gray-500 hover:text-[var(--brand-blue)]">← 안전</Link>
      <h1 className="text-2xl font-bold">안전자료실</h1>
      <ul className="bg-white rounded-2xl border border-gray-200 divide-y divide-gray-100">
        {items.length === 0 ? <li className="p-8 text-center text-gray-500">등록된 자료가 없습니다.</li> : items.map((x) => (
