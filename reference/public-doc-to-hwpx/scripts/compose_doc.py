...
def compose(input_path: str, output_path: str,
    ...
    if format_type == 'format_email':
        ...
        # 베이스 hwpx 결정 (사용자 reference 우선 → 양식 standard → Skeleton 폴백)
        base_hwpx = ''
        if reference_hwpx and Path(reference_hwpx).exists():
            base_hwpx = str(reference_hwpx)
        else:
            standard = skill_dir / 'templates' / format_type / 'standard.hwpx'
            if standard.exists():
                base_hwpx = str(standard)
        summary_info['base_mode'] = 'base_hwpx' if base_hwpx else 'skeleton_fallback'
        summary_info['base_source'] = base_hwpx or '(government/header.xml + _skeleton.hwpx)'

        title_str = payload.get('title', payload.get('cover', {}).get('title', '문서'))

        if base_hwpx:
            # 모드 B: 베이스 hwpx 통째 사용 (양식의 폰트/charPr/borderFill 모두 보존)
            cmd = [
            ...
            # 모드 A: Skeleton + government/header.xml (최후 폴백)
