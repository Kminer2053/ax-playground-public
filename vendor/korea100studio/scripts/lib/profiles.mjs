...
const SAMPLE = {
  status: {
    lead: { label: "선행", fill: "#f9edd8", border: "#c98a3a", ink: "#4a2f10", sub: "#8a5a1e" },
    key: { label: "핵심", fill: "#7a3c0c", border: "#7a3c0c", ink: "#ffe9c9", sub: "#f0cf9e" },
    normal: { label: "후속", fill: "#fdf8ee", border: "#cbb894", ink: "#2a2117", sub: "#7a6b52" },
    bottleneck: { label: "병목", fill: "#fff3e0", border: "#d9711a", ink: "#713a05", sub: "#a15208" },
    loop: { label: "회귀", fill: "#e3f2ec", border: "#2e8f76", ink: "#123d31", sub: "#24725d" },
  },
  refsLabel: "사규",
  accent: "#b4540a",
  titleOverrides: {},
  ...
  labels: {
    ...
    messageLabel: "정보 전달",
    loopLabel: "보완 회귀",
    statsTemplate: "업무 {nodes} · 담당 {lanes} · 단계 {stages}",
    groupingNoteTemplate:
      "원래 {lanes}개 담당 레인을 {groups}개 묶음으로 배치했으며, {nodes}개 업무와 {edges}개 연결 관계는 유지했습니다.",
    readingNote: "단계는 위→아래, 담당은 좌→우로 읽습니다.",
    sourceNote:
      "출처: 샘플 사규 기반 자동 생성 모델 — 실기관 데이터 아님 · 실제 결재·업무 지시가 아닙니다.",
    creditLine: "",
  },
};

const PROFILES = { default: DEFAULT, gov: GOV, sample: SAMPLE };
