...
const CARD_TITLE_SIZE = 23;
const CARD_TITLE_LINE_HEIGHT = 27;
const CARD_TITLE_DOUBLE_Y = 58;
const CARD_TITLE_SINGLE_Y = 72;
const CARD_FOOTER_SIZE = 16.5;
const CARD_FOOTER_Y = 113;
...
function renderHeader(ctx, p) {
  ...
  const titleSize = fitFontSize(board.title, 44, 30, 900);
  const subtitle = board.subtitle
    ? fitTextToWidth(board.subtitle, 900, 18)
    : "";
  ...
  return `
    <text x="1480" y="45" text-anchor="end" font-size="15" font-weight="700" fill="#8fa299">${escapeXml(stats)}</text>
    ...
    ${p.labels.creditLine ? `<text x="1480" y="214" text-anchor="end" font-size="14" font-weight="650" fill="#8fa299">${escapeXml(p.labels.creditLine)}</text>` : ""}
  `;
}
...
function renderGrid(ctx, p) {
  ...
  const result = [
    ...
    `<text x="58" y="299" font-size="19" font-weight="800" fill="#17231d">${escapeXml(p.labels.stageAxis)}</text>`,
    `<text x="58" y="332" font-size="17" font-weight="650" fill="#68776f">${escapeXml(p.labels.actorAxis)}</text>`,
  ];
  ...
  process.stages.forEach((stage, rowIndex) => {
    ...
    result.push(
      ...
      `<text x="58" y="${round(y + 32)}" class="mono" font-size="18" font-weight="800" fill="${labelInk}">${escapeXml(code)}</text>`,
      textLines(
  ...
  groups.forEach((group, groupIndex) => {
    ...
    result.push(
      ...
      `<text x="${round(x + 20)}" y="299" font-size="23" font-weight="800" fill="#17231d">${escapeXml(title)}</text>`,
  ...
  for (const edge of ctx.process.edges) {
    ...
    if (edge.label) {
      ...
      labels.push(
        ...
        `<text x="${round(label.x)}" y="${round(label.y + 5)}" text-anchor="middle" font-size="16" font-weight="750" fill="${style.color}">${escapeXml(edge.label)}</text>`
      );
    }
  }
...
function renderNode(node, ctx, p) {
  ...
  const cardW = position.width ?? CARD_WIDTH;
  const textW = cardW - 30;
  const nameLines = wrapTextToWidth(node.label, textW, CARD_TITLE_SIZE, 2);
  ...
  const fittedFooter = fitTextToWidth(footerText, textW, CARD_FOOTER_SIZE);
  const footerColor = status.sub;

  const refs = node.refs ?? [];
  const hasRefs = refs.length > 0;
  const refsRing = hasRefs
    ? `<rect x="${round(x - 4)}" y="${round(y - 4)}" width="${cardW + 8}" height="${position.height + 8}" rx="11" fill="none" stroke="${p.accent}" stroke-width="2.2" stroke-dasharray="7 5"/>`
    : "";
  const refsTag = hasRefs
    ? `<text x="${round(x + 15 + 52)}" y="${round(y + 20)}" font-size="15" font-weight="800" fill="${p.accent}">${escapeXml(`${p.refsLabel} · ${refs[0].source}`)}</text>`
    : "";

  return `
    <g filter="url(#card-shadow)">
      ${refsRing}
      <rect x="${round(x)}" y="${round(y)}" width="${cardW}" height="${position.height}" rx="8" fill="${status.fill}" stroke="${status.border}" stroke-width="2.3"/>
      <rect x="${round(x)}" y="${round(y)}" width="6" height="${position.height}" rx="3" fill="${status.border}"/>
      <text x="${round(x + 15)}" y="${round(y + 20)}" class="mono" font-size="15.5" font-weight="750" fill="${status.sub}">${escapeXml(node.id)}</text>
      ${refsTag}
      <rect x="${round(x + cardW - statusWidth - 10)}" y="${round(y + 7)}" width="${statusWidth}" height="31" rx="5" fill="${isKey ? "#ffffff" : status.border}" opacity="${isKey ? 0.18 : 0.14}"/>
      <text x="${round(x + cardW - statusWidth / 2 - 10)}" y="${round(y + 29)}" text-anchor="middle" font-size="15.5" font-weight="800" fill="${status.ink}">${escapeXml(status.label)}</text>
